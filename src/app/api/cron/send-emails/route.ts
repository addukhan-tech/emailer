...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const project of projects as Project[]) {
      // FIX 1: Check schedule first before any DB queries
      ...
      // FIX 2: Use actual value from tracker instead of hardcoded 0
      ...
      if (batchSent > 0) {
        await supabase.from('daily_send_tracker').upsert({
          project_id: project.id, date: today, emails_sent: sentToday + batchSent,
